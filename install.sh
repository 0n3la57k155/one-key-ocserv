...
check_installed_status(){
	...
	[[ ! -e ${conf} ]] && echo -e "${Error} ocserv 配置文件不存在，請檢查 !" && [[ $1 != "un" ]] && exit 1
}
check_pid(){
	if [[ ! -e ${PID_FILE} ]]; then
		PID=""
...
crl_signing_key' > ca.tmpl
	...
	[[ $? != 0 ]] && echo -e "${Error} 生成SSL證書密匙文件失敗(ca-key.pem) !" && over
	certtool --generate-self-signed --load-privkey ca-key.pem --template ca.tmpl --outfile ca-cert.pem
	[[ $? != 0 ]] && echo -e "${Error} 生成SSL證書文件失敗(ca-cert.pem) !" && over
	
	Get_ip
	if [[ -z "$ip" ]]; then
		echo -e "${Error} 檢測外網IP失敗 !"
		stty erase '^H' && read -p "請手動輸入你的服務器外網IP:" ip
		[[ -z "${ip}" ]] && echo "取消..." && over
	fi
	echo -e 'cn = "'${ip}'"
organization = "'${lalala}'"
...
tls_www_server' > server.tmpl
	...
	[[ $? != 0 ]] && echo -e "${Error} 生成SSL證書密匙文件失敗(server-key.pem) !" && over
	certtool --generate-certificate --load-privkey server-key.pem --load-ca-certificate ca-cert.pem --load-ca-privkey ca-key.pem --template server.tmpl --outfile server-cert.pem
	[[ $? != 0 ]] && echo -e "${Error} 生成SSL證書文件失敗(server-cert.pem) !" && over
...
Installation_dependency(){
	[[ ! -e "/dev/net/tun" ]] && echo -e "${Error} 你的VPS沒有開啟TUN，請聯繫IDC或通過VPS控制面板打開TUN/TAP開關 !" && exit 1
...
Install_ocserv(){
	...
	Installation_dependency
	echo -e "${Info} 開始下載/安裝 配置文件..."
	Download_ocserv
	echo -e "${Info} 開始下載/安裝 服務腳本(init)..."
	Service_ocserv
	echo -e "${Info} 開始自籤SSL證書..."
	Generate_SSL
	echo -e "${Info} 開始設置賬號配置..."
	Read_config
	Set_Config
	echo -e "${Info} 開始設置 iptables防火牆..."
	Set_iptables
...
Start_ocserv(){
	check_installed_status
	check_pid
	[[ ! -z ${PID} ]] && echo -e "${Error} ocserv 正在運行，請檢查 !" && exit 1
...
Stop_ocserv(){
	check_installed_status
	check_pid
	[[ -z ${PID} ]] && echo -e "${Error} ocserv 沒有運行，請檢查 !" && exit 1
...
Set_username(){
	echo "請輸入 要添加的VPN賬號 用户名"
	stty erase '^H' && read -p "(默認: admin):" username
	[[ -z "${username}" ]] && username="admin"
	echo && echo -e "	用户名 : ${Red_font_prefix}${username}${Font_color_suffix}" && echo
}
Set_passwd(){
	echo "請輸入 要添加的VPN賬號 密碼"
	stty erase '^H' && read -p "(默認: doub.io):" userpass
...
Set_tcp_port(){
	while true
	do
	echo -e "請輸入VPN服務端的TCP端口"
	stty erase '^H' && read -p "(默認: 443):" set_tcp_port
...
Set_udp_port(){
	while true
	do
	echo -e "請輸入VPN服務端的UDP端口"
	stty erase '^H' && read -p "(默認: ${set_tcp_port}):" set_udp_port
...
List_User(){
	[[ ! -e ${passwd_file} ]] && echo -e "${Error} ocserv 賬號配置文件不存在 !" && exit 1
	...
	if [[ ! -z ${User_text} ]]; then
		...
		do
			...
			user_list_all=${user_list_all}"用户名: "${user_name}" 賬號狀態: "${user_status}"\n"
		done
		echo && echo -e "用户總數 ${Green_font_prefix}"${User_num}"${Font_color_suffix}"
...
Add_User(){
	...
	if [[ ! -z ${user_status} ]]; then
		echo -e "${Info} 賬號添加成功 ![ ${username} ]"
	else
		echo -e "${Error} 賬號添加失敗 ![ ${username} ]" && exit 1
	fi
}
Del_User(){
	List_User
	[[ ${User_num} == 1 ]] && echo -e "${Error} 當前僅剩一個賬號配置，無法刪除 !" && exit 1
	echo -e "請輸入要刪除的VPN賬號的用户名"
	stty erase '^H' && read -p "(默認取消):" Del_username
...
Modify_User_disabled(){
	List_User
	echo -e "請輸入要啟用/禁用的VPN賬號的用户名"
	stty erase '^H' && read -p "(默認取消):" Modify_username
...
Set_Pass(){
 ...
 ${Green_font_prefix} 0.${Font_color_suffix} 列出 賬號配置
————————
 ${Green_font_prefix} 1.${Font_color_suffix} 添加 賬號配置
 ${Green_font_prefix} 2.${Font_color_suffix} 刪除 賬號配置
————————
 ${Green_font_prefix} 3.${Font_color_suffix} 啟用/禁用 賬號配置
 
 注意：添加/修改/刪除 賬號配置後，VPN服務端會實時讀取，無需重啟服務端 !" && echo
	stty erase '^H' && read -p "(默認: 取消):" set_num
	...
	if [[ ${set_num} == "0" ]]; then
		...
		echo -e "${Error} 請輸入正確的數字[1-3]" && exit 1
	fi
}
View_Config(){
	Get_ip
	...
	echo -e " 單用户設備數限制 : ${Green_font_prefix}${max_same_clients}${Font_color_suffix}"
	echo -e " 總用户設備數限制 : ${Green_font_prefix}${max_clients}${Font_color_suffix}"
	echo -e "\n 客户端鏈接請填寫 : ${Green_font_prefix}${ip}:${tcp_port}${Font_color_suffix}"
	echo && echo "==================================================="
}
View_Log(){
	[[ ! -e ${log_file} ]] && echo -e "${Error} ocserv 日誌文件不存在 !" && exit 1
	echo && echo -e "${Tip} 按 ${Red_font_prefix}Ctrl+C${Font_color_suffix} 終止查看日誌" && echo
	tail -f ${log_file}
}
Uninstall_ocserv(){
	check_installed_status "un"
	echo "確定要卸載 ocserv ? (y/N)"
	echo
	stty erase '^H' && read -p "(默認: n):" unyn
	[[ -z ${unyn} ]] && unyn="n"
	if [[ ${unyn} == [Yy] ]]; then
		check_pid
...
over(){
	...
	rm -f ocpasswd.8
	rm -f occtl.8
	echo && echo "安裝過程錯誤，ocserv 卸載完成 !" && echo
}
Add_iptables(){
...
Set_iptables(){
	...
	if [[ -z ${ifconfig_status} ]]; then
		...
		stty erase '^H' && read -p "請手動輸入你的網卡名(一般為 eth0，OpenVZ則為 venet0):" Network_card
		...
		if [[ ! -z ${Network_card} ]]; then
			...
			if [[ ! -z ${Network_card} ]]; then
				...
				ifconfig
				stty erase '^H' && read -p "檢測到本VPS的網卡非 eth0和venet0 請手動輸入你的網卡名:" Network_card
				[[ -z "${Network_card}" ]] && echo "取消..." && exit 1
			fi
		fi
	fi
...
check_sys
[[ ${release} != "debian" ]] && [[ ${release} != "ubuntu" ]] && echo -e "${Error} 本腳本不支持當前系統 ${release} !" && exit 1
...
————————————
 ...
 ${Green_font_prefix}2.${Font_color_suffix} 卸載 ocserv
————————————
...
————————————
 ${Green_font_prefix}6.${Font_color_suffix} 設置 賬號配置
 ${Green_font_prefix}7.${Font_color_suffix} 查看 配置信息
 ${Green_font_prefix}8.${Font_color_suffix} 修改 配置文件
 ${Green_font_prefix}9.${Font_color_suffix} 查看 日誌信息
————————————" && echo
if [[ -e ${file} ]]; then
	check_pid
	if [[ ! -z "${PID}" ]]; then
